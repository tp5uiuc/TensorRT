...
def conv_asym_pad_capability_validator(
    ...
    Spatial rank is read straight off the argument list -- args are
    (source, weight, bias, stride, pre_padding, post_padding, dilation, groups) -- so
    the guard holds whether or not the caller traced with dynamo, unlike the
    meta-based validators. There is no ``transposed`` argument because the pass skips
    transposed convolutions outright, so the deconvolution half of
    ``convolution_capability_validator`` has nothing to check here.
    """
    stride = args_bounds_check(node.args, 3)
