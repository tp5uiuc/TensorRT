    ...
    for gm_node in gm.graph.nodes:
        if gm_node.op == "call_module" and "_run_on_gpu" in gm_node.name:
            ...
            with gm.graph.inserting_before(gm_node):
                # Wire submodule placeholders directly to the gm-side input nodes by
                # position. gm_node.args[i] is the upstream producer for the submodule's
                # i-th placeholder, so pre-seeding val_map this way both prevents
                # graph_copy from emitting new placeholders and avoids a name-based
                # lookup that could collide with unrelated gm nodes that happen to share
                # a name with a submodule placeholder (e.g. a downstream getitem).
                ...
                # Copy the submodule body into gm at the call_module's position.
