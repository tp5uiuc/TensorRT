...
def test_bert_base_uncased_cpu_offload(ir):
    ...
    trt_mod = torchtrt.compile(model, **compile_spec)
    if ir == "dynamo":
        # offload_module_to_cpu releases the weights TensorRT absorbed. It cannot
        # release the ones the compiled module still executes in PyTorch: those are
        # the very same Parameter objects, and compile_module deliberately puts them
        # back on the target device so a model that did not fully convert stays
        # runnable. Asserting the whole source model is on the CPU only ever holds
        # for a full conversion.
        #
        # Parameters, not parameters and buffers: position_ids and token_type_ids are
        # registered persistent=False, so they never enter the exported program and
        # compile() has no handle with which to offload them. Every parameter lands in
