...
class CppSimpleModel(torch.nn.Module):
    def __init__(self):
        super().__init__()
...
def _fresh_cpp_model_and_inputs(seed=0):
    """Create a deterministic CppSimpleModel + input tensor pair for C++-runtime tests."""
    torch.manual_seed(seed)
    return CppSimpleModel().eval().cuda(), [torch.randn(2, 3, 16, 16).cuda()]


def _compile_cpp(model, inputs, runtime_cache_path=None):
    """Compile the given model through the C++ runtime path (use_python_runtime=False)."""
    kwargs = {
        "ir": "dynamo",
        "inputs": inputs,
        "enabled_precisions": {torch.float32},
...
class TestRuntimeCacheCppPersistence(TestCase):
    """Exercise the C++-runtime code path: load on engine setup, save on destructor."""
    ...
    def test_cache_saved_on_del(self):
        model, inputs = _fresh_cpp_model_and_inputs()
